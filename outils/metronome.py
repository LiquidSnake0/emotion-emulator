...
sortie = sys.argv[1] if len(sys.argv) > 1 else "metronome.wav"
bpm = float(sys.argv[2]) if len(sys.argv) > 2 else 87.85
duree = float(sys.argv[3]) if len(sys.argv) > 3 else 90.0

RATE = 48000
temps = 60.0 / bpm
n = int(RATE * duree)
buf = [0.0] * n
alea = __import__("random").Random(3)
...
while b * temps < duree - 1:
    ...
    poser(a + int(temps * RATE / 2), 9000.0, 25, 0.20, bruit=0.9)   # charley sur les croches
    b += 1
